...
WORK_DIR=/var/xintern
mkdir -p $WORK_DIR
cd $WORK_DIR

MAXIMUM_BACKUP_FILES=30              #最大备份文件数
BACKUP_FOLDERNAME="dbbackup"  #数据库备份文件的主目录
DB_HOSTNAME="localhost"              #mysql所在主机的主机名
DB_USERNAME="xintern"                   #mysql登录用户名
DB_PASSWORD="ddcreative"                 #mysql登录密码
DATABASES=(
    "xintern"
)
...
tar -czvf ${BACKUP_FOLDER}.tar.gz ${BACKUP_FOLDER} && rm -rf $BACKUP_FOLDER
BACKUP_FILES_MADE=$(ls -l ${BACKUP_FOLDERNAME}*.tar.gz | wc -l)
...
#统计所有备份文件，把最新备份的文件存放在一个临时文件里，然后删除旧的文件，循环出临时文件的备份文件从临时目录里移到当前目录
    ALL_BACKUP_FILES=($(ls -t ${BACKUP_FOLDERNAME}*.tar.gz))
...
for FILE in ${SAFE_BACKUP_FILES[@]};do
    ...
    rm -rf ${BACKUP_FOLDERNAME}*.tar.gz
    mv  -i ${FOLDER_SAFETY}/* ./
    rm -rf ${FOLDER_SAFETY}
